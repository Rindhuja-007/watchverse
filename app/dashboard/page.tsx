...
export default async function DashboardPage() {
  ...
  return <main className="min-h-screen bg-[#101214] px-6 py-8 text-[#f5f1e8] sm:px-10 lg:px-14"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-14"><div className="flex items-end justify-between"><div><p className="text-xs uppercase tracking-[0.25em] text-white/40">Your shelf</p><h2 className="mt-2 text-2xl font-semibold">Recently added</h2></div><Link href="/library" className="text-sm text-[#d9f06a]">View all</Link></div>{entries.length === 0 ? <div className="mt-6 rounded-2xl border border-dashed border-white/15 px-6 py-16 text-center"><h3 className="text-xl font-medium">Your universe is empty.</h3><p className="mt-2 text-white/50">Add your first story and make this space yours.</p></div> : <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{entries.map((entry) => <article key={entry.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04]"><div className="relative aspect-[2/3] bg-gradient-to-br from-[#3c4650] to-[#17191c]">{entry.posterPath ? <Image src={`https://image.tmdb.org/t/p/w500${entry.posterPath}`} alt={`${entry.title} poster`} fill sizes="(min-width: 1024px) 300px, 90vw" className="object-cover" /> : <div className="flex h-full items-center justify-center text-4xl text-white/20">✦</div>}</div><div className="p-5"><div className="flex items-start justify-between"><span className="rounded-full bg-[#d9f06a]/15 px-3 py-1 text-xs text-[#d9f06a]">{entry.status.replaceAll("_", " ")}</span><Star size={17} className={entry.favorite ? "fill-[#d9f06a] text-[#d9f06a]" : "text-white/30"} /></div><h3 className="mt-5 text-xl font-medium">{entry.title}</h3><p className="mt-1 text-sm text-white/45">{entry.domain} {entry.rating ? `· ${entry.rating}/10` : "· Unrated"}</p></div></article>)}</div>}</section>
  </div></main>;
}